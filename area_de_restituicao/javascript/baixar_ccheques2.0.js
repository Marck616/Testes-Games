...
function gerarPDFs() {
    ...
    for (let ano = anoInicio; ano <= anoFim; ano++) {
        ...
        for (let mes = mesInicioCorrigido; mes <= mesFimCorrigido; mes++) {
            const mesFormatado = mes.toString().padStart(2, '0'); // Formata o mês com dois dígitos
            const link = `https://rhbahia.ba.gov.br/auditor/contracheque/file/pdf/${ano}/${mesFormatado}/ZADC/${matricula}`;
            criarIframe(link);
        }
    }
}
